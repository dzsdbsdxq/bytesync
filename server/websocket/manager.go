...
func GetUserKey(roomId string, userId string) string {
	return fmt.Sprintf("%s_%s", roomId, userId)
}
...
func (manager *ClientManager) GetUserClient(roomId string, userId string) (client *Client) {
...
// GetUserList 获取用户的列表
func (manager *ClientManager) GetUserList(roomId string) (userList []string) {
...
// 向全部成员(除了自己)发送数据
func (manager *ClientManager) sendRoomIdAll(message []byte, roomId string, ignoreClient *Client) {

	clients := manager.GetUserClients()
	for _, conn := range clients {
		if conn != ignoreClient && conn.RoomId == roomId {
			conn.SendMsg(message)
		}
	}
}
...
func (manager *ClientManager) EventLogin(login *login) {
	...
	SendUserMessageAll(login.RoomId, login.UserId, orderId, models.MessageCmdEnter, "哈喽~")
}
...
func (manager *ClientManager) EventUnregister(client *Client) {
	...
	if client.UserId != "" {
		orderId := common.GetOrderIdTime()
		SendUserMessageAll(client.RoomId, client.UserId, orderId, models.MessageCmdExit, "用户已经离开~")
	}
}
...
func GetUserClient(roomId string, userId string) (client *Client) {
...
// GetUserList 获取全部用户
func GetUserList(roomId string) (userList []string) {
...
func AllSendMessages(roomId string, userId string, data string) {
